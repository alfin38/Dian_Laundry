               ...
               <tbody>
                      @foreach ($cuci_komplit as $ck)
                        <tr>
                           ...
                           <td>{{ $ck->or_number }}</td>
                           <td>{{ $ck->tgl_masuk }}</td>
                           <td>{{ $ck->nama_pelanggan }}</td>
                           <td>{{ $ck->paket->jenis_paket }}</td>
                           <td>
                              {{ \Carbon\Carbon::parse($ck->tgl_masuk)
                                 ->diffInDays(\Carbon\Carbon::parse($ck->tgl_keluar)) . ' Hari' }}
                           </td>
                           ...
                           @php
                                 $dibayarkan = $ck->dibayarkan ?? 0;
                                 $isPaid = $ck->total > 0 && $dibayarkan >= $ck->total;
                                 $statusLabel = $ck->status === 'Terlambat'
                                    ? 'Terlambat'
                                    : ($ck->status === 'Siap Diambil' ? 'Siap Diambil' : 'On Progress');
                           ...
                           <td>
                              <a href="{{ route('order.show', ['cuci-komplit', $ck->id]) }}" class="btn btn-edit">
                              ...
                              <form action="{{ route('order.destroy', $ck->id) }}" method="POST" style="display:inline-block;" data-confirm="Hapus order ini?">
                                 @csrf
                                 @method('DELETE')
