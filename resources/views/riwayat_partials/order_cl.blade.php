<div class="card">
   <div class="card-title card-flex">
      <div class="card-col">
         <h2>Daftar Transaksi - Cuci Lipat</h2>	
      </div>
   </div>
   ...
   <div class="card-body">
      <div class="tabel-kontainer">
         <table class="tabel-transaksi">
            ...
            <tbody>
               @forelse ($cuci_lipat as $dc)
                  <tr>
                     <td>{{ $loop->iteration }}</td>
                     <td>{{ $dc->or_number }}</td>
                     <td style="max-width: 150px; overflow:hidden;">{{ $dc->nama_pelanggan }}</td>
                     <td>{{ $dc->paket->nama_paket }}</td>
                     ...
                     <td>Rp.&nbsp;{{ number_format($dc->dibayarkan, 0, ',', '.') }}</td>
                     <td>Rp.&nbsp;{{ number_format($dc->kembalian, 0, ',', '.') }}</td>
                     <td><span class="success">{{ $dc->status }}</span></td>
                     <td class="align-center">
                        <a href="{{ route('order.show', ['cuci-lipat', $dc->id]) }}" class="btn btn-detail">
