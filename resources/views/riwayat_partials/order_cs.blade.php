   ...
   <div class="card-body">
      <div class="tabel-kontainer">
         <table class="tabel-transaksi">
            ...
            <tbody>
               @forelse ($cuci_satuan as $cs)
                  <tr>
                     <td>{{ $loop->iteration }}</td>
                     <td>{{ $cs->or_number }}</td>
                     <td style="max-width: 150px; overflow:hidden;">{{ $cs->nama_pelanggan }}</td>
                     <td>{{ $cs->paket->nama_paket }}</td>
                     ...
                     <td>Rp.&nbsp;{{ number_format($cs->total, 0, ',', '.') }}</td>
                     <td>Rp.&nbsp;{{ number_format($cs->dibayarkan, 0, ',', '.') }}</td>
                     <td>Rp.&nbsp;{{ number_format($cs->kembalian, 0, ',', '.') }}</td>
                     <td><span class="success">{{ $cs->status }}</span></td>
                     <td class="align-center">
                        <a href="{{ route('order.show', ['cuci-satuan', $cs->id]) }}" class="btn btn-detail">
